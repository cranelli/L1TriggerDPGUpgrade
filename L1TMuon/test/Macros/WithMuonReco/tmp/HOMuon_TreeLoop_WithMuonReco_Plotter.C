...
void HOMuon_TreeLoop_WithMuonReco_Plotter::SlaveTerminate()
{
  ...
  TFile *outfile= TFile::Open("/home/cranelli/HO_Muon/CMSSW_7_0_0/src/L1TriggerDPGUpgrade/L1TMuon/test/Plots/WithMuonReco_Plots/out_WithMuonReco_14_Muon.root","RECREATE");
    
  for(it = _h1DeltaPt.begin(); it!=_h1DeltaPt.end(); ++it){
    //cout << it->first << endl;
    it->second->Write();
    PlotCreator(_h1DeltaPt[it->first], Form("DeltaPt_%s", it->first.c_str()),"Delta Pt GeV", "Counts",-10.0, 10.0, 3, true);
  }

  for(it = _h1DeltaInvPt.begin(); it!=_h1DeltaInvPt.end(); ++it){
    //cout << it->first << endl;
    it->second->Write();
    PlotCreator(_h1DeltaInvPt[it->first], Form("DeltaInvPt_%s", it->first.c_str()),"Delta Pt GeV", "Counts",-10.0, 10.0, 3, true);
  }
...
void HOMuon_TreeLoop_WithMuonReco_Plotter::FillDeltaPtHistograms(float pt1, float pt2, std::string key){
  //Delta Pt Histograms Fill                                                                                                            
  if(!_h1DeltaPt.count(key)){
    _h1DeltaPt[key] = new TH1F(Form("DeltaPt_%s",key.c_str()), Form("DeltaPt %s",key.c_str()), 400, -5.0, 5.0);
  }
  _h1DeltaPt[key]->Fill(pt1-pt2);

  //Delta Inv Pt Histograms Fill                                                                                 
  if(!_h1DeltaInvPt.count(key)){
    _h1DeltaInvPt[key] = new TH1F(Form("DeltaInvPt_%s",key.c_str()), Form("DeltaInvPt %s",key.c_str()), 400, -10.0, 10.0);
  }
...
void HOMuon_TreeLoop_WithMuonReco_Plotter::PlotCreator(TH1F * hist, const char * title, const char * xAxis,const char * yAxis,double xMin, double xMax, int num_binmerge, bool isLogy){
...
void HOMuon_TreeLoop_WithMuonReco_Plotter::PlotCreator2D(TH2F* hist, const char* title, const char* xAxis, const char* yAxis, double xMin, double xMax,double yMin, double yMax,int num_xbinmerge, int num_ybinmerge, const char * type){
...
double HOMuon_TreeLoop_WithMuonReco_Plotter::WrapCheck(double phi1, double phi2){
