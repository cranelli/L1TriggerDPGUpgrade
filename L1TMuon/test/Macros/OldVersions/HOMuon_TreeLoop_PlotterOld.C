...
void HOMuon_TreeLoop_Plotter::PropSiPM_Map_Fill();
TH1F * hist_PropSiPMT_energy;
void HOMuon_TreeLoop_Plotter::PropSiPMT_energy_Fill();

TH1F * hist_SiPMPropTriggerT_deltaEta;
TH1F * hist_SiPMPropTriggerT_deltaPhi;
TH2F * hist_SiPMPropTriggerT_deltaEtaPhi;
void HOMuon_TreeLoop_Plotter::SiPMPropTriggerT_deltaEtadeltaPhi_Fill();
*/
...
void HOMuon_TreeLoop_Plotter::Begin(TTree * /*tree*/)
{
   ...
   hist_SiPM_energy = new TH1F("SiPM_energy_hist", "SiPM energy hist", 100,-0.1,9.9);
   ...
   hist_TightFit_energy = new TH1F("Tight_energy_hist", "Tight energy hist", 100,-0.1,9.9);
   hist_NotTightFit_energy = new TH1F("NotTight_energy_hist", "Not Tight energy hist", 100,-0.1,9.9);
   hist_TightFitT_energy = new TH1F("TightT_energy_hist", "Tight with Threshold energy Distribution", 100,-0.1,9.9);
   hist_NotTightFitT_energy = new TH1F("NotTightT_energy_hist", "Not Tight with Threshold energy Distibrution", 100,-0.1,9.9);
   hist_NotTightFit_deltaEtaPhi = new TH2F("DeltaEtaPhi_NotTightFit", "Delta Phi v Delta Eta Propagator -Trigger", 100, -2.6, 2.6, 100, -3.14, 3.14);
...
Bool_t HOMuon_TreeLoop_Plotter::Process(Long64_t entry)
{
  ...
  PropSiPMT_energy_Fill();

  //deltaEta deltaPhi Plots for SiPM Triggers for Props. 
  PropTrigger_deltaEtadeltaPhi_Fill();
...
void HOMuon_TreeLoop_Plotter::Terminate()
{
  ...
  PlotCreator(hist_SiPM_energy, "SiPM_Energy","Energy GeV", "Counts",-0.1, 5.0, 2, true);
  ...
  //PlotCreator(hist_PropSiPMT_energy, "PropSiPMT_energy", "Energy GeV","Counts", -0.1, 5.0, 1, false);
  ...
  //PlotCreator(hist_TightFit_energy, "Energy Distribution for Tight Fit", "Energy GeV", "Counts", -0.1, 5.0, 2, true);

  //PlotCreator(hist_NotTightFit_energy, "Energy Distribution for Not Tight Fit", "Energy GeV", "Counts", -0.1, 5.0, 2, true);

  //PlotCreator(hist_TightFitT_energy, "Energy Distribution for Tight Fit with Threshold", "Energy GeV", "Counts", -0.1, 5.0, 2, true);

  //PlotCreator(hist_NotTightFitT_energy, "Energy Distribution for Not Tight Fit with Threshold", "Energy GeV", "Counts", -0.1, 5.0, 2, true);
...
/*
 * Fills Histogram with all the SiPM Triggers, should include 
 * both pedestal and signal.
 */
 ...
 */
void HOMuon_TreeLoop_Plotter::PropSiPM_Map_Fill(){
  if(Propagator_IsaSiPM){
    hist_PropSiPM_Map->Fill(Propagator_Eta, Propagator_Phi);
  }
}
...
void HOMuon_TreeLoop_Plotter::PropTrigger_deltaEtadeltaPhi_Fill(){
  for(unsigned int i =0; i < Trigger_Energies->size(); i++ ){
    if(Trigger_IsaSiPMs->at(i) && (Trigger_Energies->at(i) >= 0.4)){ //SiPM Trigger Above Threshold
      hist_PropSiPMTriggerT_deltaEtaPhi->Fill(Propagator_Eta-Trigger_Etas->at(i), WrapCheck(Propagator_Phi,Trigger_Phis->at(i)));
      hist_PropTriggerSiPMT_Map->Fill(Propagator_Eta, Propagator_Phi);
      hist_GeneratorTriggerSiPMT_Map->Fill(Generator_Eta, Generator_Phi);
      if(Propagator_IsaSiPM){
	hist_SiPMPropTriggerT_deltaEta->Fill(Propagator_Eta-Trigger_Etas->at(i));
	hist_SiPMPropTriggerT_deltaPhi->Fill(WrapCheck(Propagator_Phi,Trigger_Phis->at(i)));
	hist_SiPMPropTriggerT_deltaEtaPhi->Fill(Propagator_Eta-Trigger_Etas->at(i), WrapCheck(Propagator_Phi,Trigger_Phis->at(i)));
      }
    }
  }
}
...
void HOMuon_TreeLoop_Plotter::TightFitandNot_Fill(){
